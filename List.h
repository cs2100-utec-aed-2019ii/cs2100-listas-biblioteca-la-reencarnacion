...
​
    public:
        List(List){ // Constructor copia
        }
        
        List(T*){ //Constructor  parametro, llena una lista a partir de un array
        }
​
        List(Node<T>*){ //Constructor por parametro, retorna una lista con un nodo
        }
​
        List(int){ //Constructor por parametro, retorna un lista de randoms de tamaño n
        }
​
...
​
        template<typename __T>
        inline friend ForwardList& operator>> (ForwardList<__T>&, const T& ) = 0; = 0; // pop_back de un elemento
}
​